...
public class InService {
	...
	public Long getIncome(Long userId){
		...
		try {
			con = DBUtil.getConnection();
			//sumIncome = dao.getTotalIncome(con,userId);
		} catch (Exception e) {
			e.printStackTrace();
		} finally{
			try {
	...
	//按时间查询收入
	public List<Book> getInByTime(int startNum, int endNum, String email, String times) {
		if (email == null || email.equals("")) {
			return null;
		}
		List<Book> accountList2 = new ArrayList<Book>();
	 ...
	 * @return
	 */
	public int getInSize(String email, String times) {
		PersistenceManager pm = PMF.get().getPersistenceManager();
		Query query = null;
		try {
			...
			if(times != null && !times.equals("")) {
				...
				accountList = (List<Book>) pm.newQuery(query).execute(today);
			} else {
